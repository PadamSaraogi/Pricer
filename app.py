...
# -------- Beginner-friendly summary helpers --------
def _format_bullets(lines):
    """Keep at most 4–5 short bullets, drop empties."""
    clean = [ln for ln in lines if ln]
    if not clean:
        return ""
    return "### 🔰 Quick interpretation\n\n" + "\n".join(f"- {ln}" for ln in clean[:5])


def make_option_summary(opts, call, put, d1, d2, G, tag_info, mkt_price, iv_single, iv_mid):
    lines = []
    S = opts["S_for_options"]
    K = opts["K"]
    opt_label = opts["opt_type"]
    tag = tag_info.get("tag", "")

    # 1) Moneyness & prices
    if "ATM" in tag:
        lines.append(f"{opt_label} is **at-the-money**: S₀,eff ≈ K ({S:.2f} vs {K:.2f}).")
    elif "ITM" in tag:
        lines.append(f"{opt_label} is **in-the-money**: S₀,eff is favourable vs K ({S:.2f} vs {K:.2f}).")
    elif "OTM" in tag:
        lines.append(f"{opt_label} is **out-of-the-money**: most of the value is **time value**, not intrinsic.")
    else:
        lines.append(f"Moneyness tag: {tag} — double-check S₀ and K if this looks odd.")

    lines.append(f"Model prices: Call ≈ {call:.2f}, Put ≈ {put:.2f} using the same inputs.")

    # 2) Sensitivity cluster (Delta + Gamma)
    if opt_label == "Call":
        delta = G["delta"]["call"]
        theta = G["theta_per_day"]["call"]
    else:
        delta = G["delta"]["put"]
        theta = G["theta_per_day"]["put"]
    gamma = G["gamma"]
    vega_1pct = G["vega_per_1pct"]
    ...
    gamma_speed = "fast" if gamma > 0.05 else "slow"
    lines.append(
        f"Delta ≈ {delta:.2f}, Gamma ≈ {gamma:.3f} → {sens_word} sensitivity to the underlying, with {gamma_speed} changes in delta if the price moves."
    )

    # 3) Time + vol cluster (Theta + Vega)
    decay = "loses value each day (time decay)" if theta < 0 else "has almost no time decay"
    if vega_1pct > 0.5:
        vol_word = "very sensitive to volatility"
    elif vega_1pct > 0.1:
        vol_word = "moderately affected by volatility"
    else:
        vol_word = "not very sensitive to volatility"

    lines.append(
        f"Theta ≈ {theta:.4f}/day → the option {decay}. Vega ≈ {vega_1pct:.3f} per 1% vol → {vol_word}."
    )

    # 4) Market vs model + IV (only if user gives prices)
    model_px = call if opt_label == "Call" else put
    if mkt_price and mkt_price > 0 and model_px > 0:
        diff = (mkt_price - model_px) / model_px
        if abs(diff) < 0.05:
            lines.append(f"Market price {mkt_price:.2f} is **close** to model price {model_px:.2f}.")
        elif diff > 0:
            lines.append(
                f"Market price {mkt_price:.2f} is **above** model price {model_px:.2f} → market is pricing in **extra risk/volatility**."
            )
        else:
            lines.append(
                f"Market price {mkt_price:.2f} is **below** model price {model_px:.2f} → model suggests it may be **cheap**, or your inputs differ from the market."
            )

    iv_used = iv_single or iv_mid
    if iv_used is not None:
        lines.append(f"Implied volatility ≈ {iv_used*100:.1f}% → market’s view of future volatility.")

    return _format_bullets(lines)


def make_bond_summary(price, face, coupon_rate, ytm, mac, mod, conv):
    lines = []
    if abs(price - face) < 0.5:
        rel = "near **par**"
        msg = "coupon ≈ market yield."
    elif price > face:
        rel = "at a **premium**"
        msg = "coupon is **above** market yield."
    else:
        rel = "at a **discount**"
        msg = "coupon is **below** market yield."

    lines.append(f"Price ≈ {price:.2f} vs face {face:.2f} → bond is {rel}; {msg}")
    lines.append(f"YTM ≈ {ytm*100:.2f}% → approximate annual return if you hold to maturity.")
    lines.append(
        f"Duration: Macaulay ≈ {mac:.2f}, modified ≈ {mod:.2f} → about {mod:.2f}% price change for a 1% move in yields."
    )
    lines.append(f"Convexity ≈ {conv:.2f} → higher convexity makes this duration-based estimate more accurate.")
    return _format_bullets(lines)


def make_swap_summary(par_rate, fixed_rate, pv, dv01_val, notional, T, freq):
    lines = []
    lines.append(
        f"Par rate ≈ {par_rate*100:.3f}%, your fixed rate ≈ {fixed_rate*100:.3f}%."
    )
    if abs(pv) < 1e-6:
        lines.append("Swap PV is ~0 → this is roughly a **fair (par) swap**.")
    elif pv > 0:
        lines.append(
            f"PV ≈ {pv:,.2f} (Fixed − Float) **positive** → receiving fixed at your rate is **valuable** vs the curve."
        )
    else:
        lines.append(
            f"PV ≈ {pv:,.2f} (Fixed − Float) **negative** → paying fixed at your rate is **advantageous**; receiving fixed is unattractive."
        )

    lines.append(
        f"DV01 ≈ {dv01_val:,.2f} per 1 bp → a 0.01% parallel rate move changes PV by this amount on notional {notional:,.0f}."
    )
    lines.append(
        f"Maturity ≈ {T:.1f} years with {freq} payments/year → longer maturity and larger DV01 means more interest-rate risk."
    )
    return _format_bullets(lines)


def make_curve_summary(curve: YieldCurve):
    lines = []
    mats = curve.mats
    zeros = curve.zeros
    if len(mats) >= 2:
        ...
        elif long + 0.10 < short:
            slope = "inverted"
            msg = "long-term yields are **below** short-term yields (often seen as a recession signal)."
        else:
            slope = "flat"
            msg = "little difference between short and long rates."
        lines.append(
            f"Curve is {slope}: short end ≈ {short:.2f}%, long end ≈ {long:.2f}% → {msg}"
        )

    if len(mats) >= 3:
        mid_i = len(mats) // 2
        lines.append(
            f"Mid-range rate around T ≈ {mats[mid_i]:.1f} yrs is ≈ {zeros[mid_i]*100:.2f}%."
        )

    lines.append(
        "Use the forward-rate box below to see the implied short-term rate between any two maturities."
    )
    return _format_bullets(lines)
...
with tab1:
    ...
    if mkt_price > 0:
        ...
        coliv1.metric(
            "IV (single price)" if iv is not None else "IV error",
            f"{iv*100:.3f}%" if iv is not None else "No root",
        )
    if mid is not None:
        iv_mid = implied_vol(mid, inp, otype)
        coliv2.metric(
            "IV (Bid/Ask mid)" if iv_mid is not None else "IV error",
            f"{iv_mid*100:.3f}%" if iv_mid is not None else "No root",
        )

    # 🔰 Compact, output-based explanation for this option
    summary_md = make_option_summary(
        opts=opts,
        call=call,
        put=put,
        d1=d1,
    ...
    with st.expander("Beginner notes (this option)", expanded=True):
...
with tab5:
    ...
    if mode_bond_input == "Numeric T":
        ...
        if mode_bond == "Inputs → Price":
            ...
            # 🔰 Beginner-friendly summary for this bond (numeric inputs)
            summary_md_bond = make_bond_summary(P, face, coupon_pct, ytm_pct, mac, mod, conv)
            with st.expander("Beginner notes (this bond)", expanded=True):
                st.markdown(summary_md_bond)
            ...
            if ytm_solved is None:
                ...
                # 🔰 Beginner-friendly summary when solving YTM from price
                summary_md_bond = make_bond_summary(target_price, face, coupon_pct, ytm_solved, mac, mod, conv)
                with st.expander("Beginner notes (this bond)", expanded=True):
                    st.markdown(summary_md_bond)
...
with tab6:
    ...
    if curve is not None:
        ...
        # 🔰 Beginner-friendly interpretation of the yield curve
        curve_summary = make_curve_summary(curve)
        with st.expander("Beginner notes (yield curve)", expanded=True):
            st.markdown(curve_summary)
...
with tab7:
    ...
    if curve is None:
        ...
        st.metric("Swap PV (Fixed - Float)", f"{pv_swap:,.2f}")
        st.metric("DV01 (per +1bp shift)", f"{dv01_val:,.2f}")

        # 🔰 Beginner-friendly swap interpretation
        swap_summary = make_swap_summary(
            par_rate=s_par,
        ...
        with st.expander("Beginner notes (this swap)", expanded=True):
            st.markdown(swap_summary)
