...
def _figure_title_page(title: str, subtitle: str) -> plt.Figure:
    fig = plt.figure(figsize=(8.3, 11.7))  # A4 portrait-ish
...
def _figure_smile_plot(S0_eff: float, r: float, q: float, T: float, sigma_seed: float, K_center: float) -> plt.Figure:
    ...
    ax.set_xlabel("Strike (K)"); ax.set_ylabel("Option value"); ax.set_title("Option value vs Strike at current σ")
    ax.legend()
    return fig
...
def build_options_report_pdf(
    *,
    ...
    """
    Compose a multi-page PDF: Title, Inputs, Prices/Greeks, Smile, Curve (if any).
    Returns PDF bytes.
    """
    ...
    with PdfPages(buf) as pdf:
        ...
        # Prices & Greeks (two tables)
        fig_pg = plt.figure(figsize=(8.3, 8.3))
        ax1 = fig_pg.add_subplot(2, 1, 1); ax1.axis("off"); ax1.set_title("Prices", fontsize=14, pad=8)
        tbl1 = ax1.table(cellText=[[k, v] for k, v in prices.items()], colLabels=["Metric", "Value"], loc="center")
        tbl1.auto_set_font_size(False); tbl1.set_fontsize(10); tbl1.scale(1.1, 1.3)

        ax2 = fig_pg.add_subplot(2, 1, 2); ax2.axis("off"); ax2.set_title("Greeks", fontsize=14, pad=8)
        tbl2 = ax2.table(cellText=[[k, v] for k, v in greeks.items()], colLabels=["Greek", "Value"], loc="center")
        tbl2.auto_set_font_size(False); tbl2.set_fontsize(10); tbl2.scale(1.1, 1.3)
        pdf.savefig(fig_pg); plt.close(fig_pg)

        # Smile-ish value vs strike
